...
`;
    ...
    ISFRenderer$1.prototype.loadSource = function loadSource(fragmentISF, vertexISFOpt) {
    ...
    ISFRenderer$1.prototype.sourceChanged = function sourceChanged(fragmentShader, vertexShader, model) {
    ...
    ISFRenderer$1.prototype.initUniforms = function initUniforms() {
    ...
    ISFRenderer$1.prototype.setValue = function setValue(name, value) {
      this.program.use();
    ...
    ISFRenderer$1.prototype.setNormalizedValue = function setNormalizedValue(name, normalizedValue) {
      const inputs = this.model.inputs;
      let input = null;
      for (let i = 0; i < inputs.length; i++) {
    ...
    ISFRenderer$1.prototype.setupPaintToScreen = function setupPaintToScreen() {
    ...
    ISFRenderer$1.prototype.setupGL = function setupGL() {
      this.cleanup();
    ...
    ISFRenderer$1.prototype.generatePersistentBuffers = function generatePersistentBuffers() {
      this.renderBuffers = [];
    ...
    ISFRenderer$1.prototype.paintToScreen = function paintToScreen(destination, target) {
    ...
    ISFRenderer$1.prototype.pushTextures = function pushTextures() {
    ...
    ISFRenderer$1.prototype.pushTexture = function pushTexture(uniform) {
      if (!uniform.value) {
        return;
      }
    ...
    ISFRenderer$1.prototype.pushUniforms = function pushUniforms() {
      for (const uniform of this.uniforms) {
        this.pushUniform(uniform);
      }
    };

    ISFRenderer$1.prototype.pushUniform = function pushUniform(uniform) {
      const loc = this.program.getUniformLocation(uniform.name);
      if (loc !== -1) {
        if (uniform.type === 't') {
    ...
    ISFRenderer$1.prototype.findUniforms = function findUniforms(shader) {
    ...
    ISFRenderer$1.prototype.typeToUniform = function typeToUniform(type) {
      switch (type) {
        case 'float':
          return {
            type: 'f',
    ...
    ISFRenderer$1.prototype.setDateUniforms = function setDateUniforms() {
      const now = Date.now();
    ...
    ISFRenderer$1.prototype.draw = function draw(destination) {
    ...
    ISFRenderer$1.prototype.evaluateSize = function evaluateSize(destination, formula) {
      formula += '';
    ...
    ISFRenderer$1.prototype.cleanup = function cleanup() {
      this.contextState.reset();
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(canvas, file, 100, 1, 2530);
    			add_location(main, file, 99, 0, 2522);
    		},
